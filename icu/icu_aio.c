...
// next must be included if not using #include "uresb.h"
...
L10nKeys* gtkeys_create() {
	...
	(gtkeys)->wdw_title = "wdw_title";
	(gtkeys)->greeting = "greeting";
	(gtkeys)->farewell = "farewell";
	(gtkeys)->welcome = "welcome";
	(gtkeys)->lbl_lang = "lbl_lang";
	(gtkeys)->cmd_lang_root = "cmd_lang_root";
	(gtkeys)->cmd_lang_ar = "cmd_lang_ar";
	(gtkeys)->cmd_lang_de = "cmd_lang_de";
	(gtkeys)->cmd_lang_en = "cmd_lang_en";
	(gtkeys)->cmd_lang_es = "cmd_lang_es";
	(gtkeys)->cmd_lang_fr = "cmd_lang_fr";
	(gtkeys)->cmd_lang_hi = "cmd_lang_hi";
	(gtkeys)->cmd_lang_ja = "cmd_lang_ja";
	(gtkeys)->cmd_lang_ru = "cmd_lang_ru";
	(gtkeys)->cmd_lang_sr = "cmd_lang_sr";
	(gtkeys)->cmd_lang_zh = "cmd_lang_zh";
	(gtkeys)->cmd_lang_zh_TW = "cmd_lang_zh_TW";
	(gtkeys)->nlangs = "nlangs";
	(gtkeys)->nlangs_plural = "nlangs_plural";
	return (gtkeys);
}
...
void update_langs_avail(L10nKeys *gtkeys, int nlangs, char *langs_usrt[nlangs], char *packageName, char *locale, UFILE *outerr) {//, char *packageName, UFILE *out, UFILE *outerr) {
	...
	*/
	langs_usrt[0] = str_by_key(packageName, locale, gtkeys->cmd_lang_root, outerr);
	langs_usrt[1] = str_by_key(packageName, locale, gtkeys->cmd_lang_ar, outerr);
	langs_usrt[2] = str_by_key(packageName, locale, gtkeys->cmd_lang_de, outerr);
	langs_usrt[3] = str_by_key(packageName, locale, gtkeys->cmd_lang_en, outerr);
	langs_usrt[4] = str_by_key(packageName, locale, gtkeys->cmd_lang_es, outerr);
	langs_usrt[5] = str_by_key(packageName, locale, gtkeys->cmd_lang_fr, outerr);
	langs_usrt[6] = str_by_key(packageName, locale, gtkeys->cmd_lang_hi, outerr);
	langs_usrt[7] = str_by_key(packageName, locale, gtkeys->cmd_lang_ja, outerr);
	langs_usrt[8] = str_by_key(packageName, locale, gtkeys->cmd_lang_ru, outerr);
	langs_usrt[9] = str_by_key(packageName, locale, gtkeys->cmd_lang_sr, outerr);
	langs_usrt[10] = str_by_key(packageName, locale, gtkeys->cmd_lang_zh, outerr);
	langs_usrt[11] = str_by_key(packageName, locale, gtkeys->cmd_lang_zh_TW, outerr);
}
...
int main(int argc, char *argv[]) {
	...
	*/
	...
	UFILE *out = (NULL); // used by u_fprintf to print localized messages to stdout
	UFILE *outerr = (NULL); // used by u_fprintf to print localized messages to stderr
	...
	//char *langs_avail[LANGS_AVAILABLE] = {"root", "ar", "de", "en", "es", "fr", "hi", "ja", "ru", "sr", "zh", "zh_TW"};
	...
	*/
	...
	char *icu_obj_resources = "/resources/i18n-icu";
	while(!str_concat((&currcpy), (icu_obj_resources)));
	packageName = currcpy;
	//printf("%s\n", currdir);
	//printf("%s\n", packageName);
	
	out = u_finit(stdout, locale, encoding); // used by u_fprintf to print localized messages to stdout
	outerr = u_finit(stderr, locale, encoding); // used by u_fprintf to print localized messages to stderr
	
	L10nKeys *gtkeys = (NULL);
	gtkeys = gtkeys_create();
	
	char *key = gtkeys->welcome; // key string to get the wanted localized value
	char *key_str = str_by_key(packageName, locale_default, key, outerr);
	if (!(key_str)) free(key_str);
	key = (langs_total == 1) ? (gtkeys)->nlangs : gtkeys->nlangs_plural; // key string to get the wanted localized value
	key_str = str_by_key(packageName, locale_default, key, outerr);
	printf(key_str, langs_total);
	printf("\n");
	if (!(key_str)) free(key_str);
	/*
	// this for loop requires to include utils for i18n/i10n resources in icu like #include "uresb.h"
	for(int i = 1; i < argc; ++i) {
		if (is_lang_avail(langs_total, langs_avail, argv[i], &cmb_lang_active)) {
			locale = argv[i];
			u_fprintf(out, "uresb: processing file \"%s\" in path \"%s\"\n", locale, packageName);
			UErrorCode status = U_ZERO_ERROR; // number for error code by status
			UResourceBundle *bundle = ures_open(packageName, locale, &status); // bundle for locale resource
			if(U_SUCCESS(status)) {
				u_fprintf(out, "%s\n", locale);
				int32_t indent = 0; // variable to increment tabulation when printing lines
				printOutBundle(bundle, packageName, locale, indent, &status, out, outerr);
			} else {
				reportError(packageName, locale, &status, outerr);
			}
			ures_close(bundle);
		}
	}
	*/
	for(int i = 1; i < argc; ++i) {
		if (is_lang_avail(langs_total, langs_avail, argv[i], &cmb_lang_active)) {
			...
			char *key_str = str_by_key(packageName, locale, key, outerr);
			if (!(key_str)) free(key_str);
		}
	}
	...
	update_langs_avail(gtkeys, langs_total, langs_usrt, packageName, locale, outerr);//, packageName, out, outerr);
